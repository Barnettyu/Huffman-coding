...
void HuffmanEncoding( char *OrigMsg, char *EncodingMsg, float *pCharFreq, int NumOfCharSet, HuffmanTreeNode *HuffmanTree, HuffmanCodeNode *HuffmanCodeBook );
...
void HuffmanEncoding( char *OrigMsg, char *EncodingMsg, float *pCharFreq, int NumOfCharSet, HuffmanTreeNode *HuffmanTree, HuffmanCodeNode *HuffmanCodeBook )
{
  ...
  CountEncodingMsgCharFrequency( OrigMsg, pCharFreq, NumOfCharSet );

  //////////////////////////////////////////////////////////////
  printf( "\n\t待编码消息中信源字符的频度 :\n\t" );
  for ( k = 0; k < NumOfCharSet; k++ )
	printf( "%f ", *( pCharFreq + k ) );
  printf( "\n" );
  //////////////////////////////////////////////////////////////
  ...
  for ( k = 0; k < ( 2 * NumOfCharSet - 1 ); k++ )
  {
	...
	if ( k < NumOfCharSet )
	  ( HuffmanTree + k ) -> Weight = *( pCharFreq + k );
	else
  ...
  while ( !IsStop )
  {
	...
	if ( strlen( OrigMsg ) > 0 )
	{
      ...
      if ( HuffmanTree != NULL )
      {
	    ...
	    {
		  // 进行 'Huffman' 编码 ...
          HuffmanEncoding( OrigMsg, EncodingMsg, pMsgCharFrequency, NumOfCharSet, HuffmanTree, HuffmanCodeBook );
